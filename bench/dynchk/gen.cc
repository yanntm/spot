...
int main(int argc, char **argv)
{
  ...
  do
	...
	break;
      ...
      const spot::kripke *system = spot::load_dve2
	(dve_model_name, dict, &ap, spot::ltl::constant::true_instance(),
	 2 /* 0 or 1 or 2 */ , true);
      assert(system);
	  ...
	  assert(A_st || A_tr || A_str || A_acc);
	      ...
	      // Compute stats for Tmin
      	      spot::tgba_sub_statistics sstat_tmin =
		sub_stats_reachable(mterminal);
	      ...
	      // Compute stats for Tmin product
      	      const spot::tgba *p_mterminal =
		new spot::tgba_product(system, mterminal);
	      ...
	      assert(TminxK_st || TminxK_tr);
      	  ...
      	  if (SminxK_st >= SIZE || WminxK_st >= SIZE || TminxK_st >= SIZE)
      		...
      		{
		  // Compute stats for S
		  spot::tgba_sub_statistics sstat_s =
		    sub_stats_reachable(strong);
		  S_st = sstat_s.states;
		  ...
		  // Compute stats for S product
      		  const spot::tgba *p_strong =
		    new spot::tgba_product(system, strong);
		  ...
		  // Compute stats for W product
      		  const spot::tgba *p_weak =
		    new spot::tgba_product(system, weak);
		  ...
		  assert(WxK_st || WxK_tr);
      		...
      		{
		  // Compute stats for T
		  spot::tgba_sub_statistics sstat_t =
		    sub_stats_reachable(terminal);
		  T_st = sstat_t.states;
		  ...
		  // Compute stats for T product
      		  ...
      		  spot::tgba_statistics s_terminal =
		    stats_reachable(p_terminal);
	      ...
	      if (product)
		{
		  ...
		  const char* err;
		  echeck_inst =
		    spot::emptiness_check_instantiator::construct
		    ( "Cou99", &err);
		  if (!echeck_inst)
		    {
		      std::cerr << "ERROR\n";
