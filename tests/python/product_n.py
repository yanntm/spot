...
for i in range(42):
    for nb_formula in range(1, 7):
        ...
        for nb_ap in range(2,5):
            ...
            src = "spot.randltl(" + str(nb_ap) + ", " + str(nb_formula) + ")"
            # Convert them to automata
            auts = [g.translate() for g in f]

            # Compute the n-ary product
            prod = spot.product_n(auts)

            assert prod.is_empty() == \
                   spot.formula_And(f).translate().is_empty(), src

            # Compute the n-ary product of the the negated automata
            neg = spot.formula_Or([spot.formula_Not(g) for g in f]).translate()

            assert spot.product(prod, neg).is_empty(), src
