...
const unsigned BLOEMEN_SAFETY = 263;
const unsigned LAARMAN_SOURCE = 264;
const unsigned LAARMAN_SAFETY = 265;

const unsigned COND_DEST_LIVENESS = 266;
const unsigned COND_DEST_SAFETY = 267;
...
struct mc_options_
{
  ...
  bool laarman_source = false;
  bool laarman_safety = false;
  bool none = false;
  bool use_por = false;
...
parse_opt_finput(int key, char* arg, struct argp_state*)
{
  ...
  switch (key)
    ...
    case 'b':
      ...
      break;
    case BLOEMEN_SAFETY:
      mc_options.bloemen_safety = true;
      break;
    ...
    case COND_DEST:
      ...
      break;
    case COND_DEST_LIVENESS:
      mc_options.cond_dest_liveness = true;
      break;
    case COND_DEST_SAFETY:
      mc_options.cond_dest_safety = true;
      break;
    ...
    case NONE:
      ...
      break;
    case LAARMAN_SOURCE:
      mc_options.laarman_source = true;
      break;
    case LAARMAN_SAFETY:
      mc_options.laarman_safety = true;
      break;
    case COUNTEREXAMPLE:
...
static const argp_option options[] =
  {
    ...
    { "bloemen_safety", BLOEMEN_SAFETY, nullptr, 0,
      "run the SCC computation of Bloemen et al. (PPOPP'16) for safety", 0 },
    ...
    { "laarman_source", COND_SOURCE, nullptr, 0,
      "run the blue walk of CNDFS POR algorithm (with --POR)", 0 },
    { "laarman_safety", COND_SOURCE, nullptr, 0,
      "run the safety exploration proposed by Laarman et al. (with --POR)", 0 },
...
static int checked_main()
{
  ...
  if (mc_options.nb_threads == 1 &&
    ...
    if (mc_options.bloemen_safety &&  mc_options.model != nullptr)
      {
        unsigned int hc = std::thread::hardware_concurrency();
        if (mc_options.nb_threads > hc)
          std::cerr << "Warning: you require " << mc_options.nb_threads
        ...
        int memused = spot::memusage();
        tm.start("bloemen_safety");
        auto res = spot::bloemen_safety<spot::ltsmin_kripkecube_ptr,
                                 spot::cspins_state,
                                 spot::cspins_iterator,
                                 spot::cspins_state_hash,
                                 spot::cspins_state_equal>(modelcube);
        tm.stop("bloemen_safety");
        memused = spot::memusage() - memused;
      ...
      if (mc_options.csv)
        {
          ...
          std::cout << '#'
                    << split_filename(mc_options.model)
                    << ','
                    << tm.timer("bloemen_safety").walltime() << ','
                    << memused << ','
                    << inserted << ','
                    << st << ','
                    << tr << ','
        ...
        if (mc_options.bloemen_nosharing &&  mc_options.model != nullptr)
      {
        ...
        int memused = spot::memusage();
        tm.start("bloemen_nosharing");
        auto res = spot::bloemen_nosharing<spot::ltsmin_kripkecube_ptr,
                                 spot::cspins_state,
                                 spot::cspins_iterator,
                                 spot::cspins_state_hash,
                                 spot::cspins_state_equal>(modelcube);
        tm.stop("bloemen_nosharing");
        memused = spot::memusage() - memused;
      ...
      if (mc_options.csv)
        {
          ...
          std::cout << '#'
                    << split_filename(mc_options.model)
                    << ','
                    << tm.timer("bloemen_nosharing").walltime() << ','
                    << memused << ','
                    << inserted << ','
                    << st << ','
                    << tr << ','
    ...
    if (mc_options.cond_dest_liveness &&  mc_options.model != nullptr)
      {
        unsigned int hc = std::thread::hardware_concurrency();
        if (mc_options.nb_threads > hc)
          std::cerr << "Warning: you require " << mc_options.nb_threads
        ...
        int memused = spot::memusage();
        tm.start("cond_dest_liveness");
        auto res = spot::cond_dest_liveness<spot::ltsmin_kripkecube_ptr,
                                 spot::cspins_state,
                                 spot::cspins_iterator,
                                 spot::cspins_state_hash,
                                 spot::cspins_state_equal>(modelcube);
        tm.stop("cond_dest_liveness");
        memused = spot::memusage() - memused;
        ...
        if (mc_options.csv)
          {
            ...
            std::cout << '#'
                      << split_filename(mc_options.model)
                      << ','
                      << tm.timer("cond_dest_liveness").walltime() << ','
                      << memused << ','
                      << inserted << ','
                      << st << ','
                      << tr << ','
    ...
    if (mc_options.cond_dest_safety &&  mc_options.model != nullptr)
      {
        unsigned int hc = std::thread::hardware_concurrency();
        if (mc_options.nb_threads > hc)
          std::cerr << "Warning: you require " << mc_options.nb_threads
        ...
        int memused = spot::memusage();
        tm.start("cond_dest_safety");
        auto res = spot::cond_dest_safety<spot::ltsmin_kripkecube_ptr,
                                          spot::cspins_state,
                                          spot::cspins_iterator,
                                          spot::cspins_state_hash,
                                          spot::cspins_state_equal>(modelcube);
        tm.stop("cond_dest_safety");
        memused = spot::memusage() - memused;
        ...
        if (mc_options.csv)
          {
            ...
            std::cout << '#'
                      << split_filename(mc_options.model)
                      << ','
                      << tm.timer("cond_dest_safety").walltime() << ','
                      << memused << ','
                      << inserted << ','
                      << st << ','
                      << tr << ','
    ...
    if (mc_options.rsscc_nosharing &&  mc_options.model != nullptr)
      {
        unsigned int hc = std::thread::hardware_concurrency();
        if (mc_options.nb_threads > hc)
          std::cerr << "Warning: you require " << mc_options.nb_threads
        ...
        int memused = spot::memusage();
        tm.start("rsscc_nosharing");
        auto res = spot::rsscc_nosharing<spot::ltsmin_kripkecube_ptr,
                                         spot::cspins_state,
                                         spot::cspins_iterator,
                                         spot::cspins_state_hash,
                                         spot::cspins_state_equal>(modelcube);
        tm.stop("rsscc_nosharing");
        memused = spot::memusage() - memused;
    ...
    if (mc_options.rsscc &&  mc_options.model != nullptr)
      {
        unsigned int hc = std::thread::hardware_concurrency();
        if (mc_options.nb_threads > hc)
          std::cerr << "Warning: you require " << mc_options.nb_threads
        ...
        int memused = spot::memusage();
        tm.start("rsscc");
        auto res = spot::rsscc<spot::ltsmin_kripkecube_ptr,
                               spot::cspins_state,
                               spot::cspins_iterator,
                               spot::cspins_state_hash,
                               spot::cspins_state_equal>(modelcube);
    ...
    if (mc_options.laarman_source &&  mc_options.model != nullptr)
      {
        unsigned int hc = std::thread::hardware_concurrency();
        if (mc_options.nb_threads > hc)
          std::cerr << "Warning: you require " << mc_options.nb_threads
        ...
        int memused = spot::memusage();
        tm.start("laarman_source");
        auto res = spot::laarman_source<spot::ltsmin_kripkecube_ptr,
                                     spot::cspins_state,
                                     spot::cspins_iterator,
                                     spot::cspins_state_hash,
                                     spot::cspins_state_equal>(modelcube);
        tm.stop("laarman_source");
        memused = spot::memusage() - memused;
        ...
        if (mc_options.csv)
          {
            ...
            std::cout << '#'
                      << split_filename(mc_options.model)
                      << ','
                      << tm.timer("laarman_source").walltime() << ','
                      << memused << ','
                      << inserted << ','
                      << st << ','
                      << tr << ','
    ...
    if (mc_options.laarman_safety &&  mc_options.model != nullptr)
      {
        unsigned int hc = std::thread::hardware_concurrency();
        if (mc_options.nb_threads > hc)
          std::cerr << "Warning: you require " << mc_options.nb_threads
        ...
        int memused = spot::memusage();
        tm.start("laarman_safety");
        auto res = spot::laarman_safety<spot::ltsmin_kripkecube_ptr,
                                     spot::cspins_state,
                                     spot::cspins_iterator,
                                     spot::cspins_state_hash,
                                     spot::cspins_state_equal>(modelcube);
        tm.stop("laarman_safety");
        memused = spot::memusage() - memused;
        ...
        if (mc_options.csv)
          {
            ...
            std::cout << '#'
                      << split_filename(mc_options.model)
                      << ','
                      << tm.timer("laarman_safety").walltime() << ','
                      << memused << ','
                      << inserted << ','
                      << st << ','
                      << tr << ','
