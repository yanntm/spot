...
#define NEW_AUT() do {                                        \
   result.emplace_back(spot::random_graph(3, 0.5, &apf,       \
                       current_bdd, 0, 0, 0.5, true), 0);     \
  LAST_AUT->prop_state_acc(false);                            \
  LAST_AUT->prop_weak(false);                                 \
  LAST_AUT->prop_inherently_weak(false);                      \
  LAST_AUT->prop_stutter_invariant(spot::trival::maybe());    \
  for (auto& t: LAST_AUT->edges())                            \
    t.acc = spot::acc_cond::mark_t();                         \
  LAST_NUM_SETS = 0;                                          \
} while (false)
...
{
  auto tmp = spot::dtwa_complement(right);
...
static int order2num(spot::parity_order order)
{
  switch (order)
    {
      case spot::parity_order_min:
        return 0;
      case spot::parity_order_max:
        return 1;
      case spot::parity_order_same:
        return 2;
      case spot::parity_order_dontcare:
        return 3;
    }
  return 4;
}

static int style2num(spot::parity_style style)
{
  switch (style)
    {
      case spot::parity_style_odd:
        return 0;
      case spot::parity_style_even:
        return 1;
      case spot::parity_style_same:
        return 2;
      case spot::parity_style_dontcare:
        return 3;
    }
  return 4;
}
...
static bool is_right_parity(spot::const_twa_graph_ptr aut,
                            spot::parity_order target_order,
                            ...
                            bool origin_max, bool origin_odd, unsigned num_sets)
{
  ...
  if (aut->num_sets() <= 1 || num_sets <= 1
      || target_order == spot::parity_order_dontcare)
    target_max = is_max;
  else if (target_order == spot::parity_order_max)
    target_max = true;
  else if (target_order == spot::parity_order_min)
    target_max = false;
  else
    target_max = origin_max;
  if (aut->num_sets() == 0 || num_sets == 0
      || target_style == spot::parity_style_dontcare)
    target_odd = is_odd;
  ...
  if (!(is_max == target_max && is_odd == target_odd))
    {
      ...
      std::string order[] = { "max", "min", "same", "dontcare" };
      std::string style[] = { "odd", "even", "same", "dontcare" };
      std::cerr << "target: " << order[order2num(target_order)] << ' '
                << style[style2num(target_style)] << std::endl;
      std::cerr << "origin: " << order[origin_max ? 0 : 1] << ' '
                << style[origin_odd ? 0 : 1] << ' '
                << num_sets << std::endl;
      std::cerr << "actually: " << order[is_max ? 0 : 1] << ' '
                << style[is_odd ? 0 : 1] << ' '
...
static bool is_colorized(spot::const_twa_graph_ptr aut, bool hard)
{
  for (auto t: aut->edges())
    if (t.acc.count() > 1 && (t.acc.count() != 0 || hard))
      {
        std::cerr << "======Not colorized======" << std::endl;
...
int main()
{
  ...
  auto parity_orders =
  {
    spot::parity_order_max,
    spot::parity_order_min,
    spot::parity_order_same,
    spot::parity_order_dontcare,
  };
  auto parity_styles =
  {
        ...
        if (aut_num_sets <= acc_num_sets)
          {
            ...
            // Check change_parity
            for (auto order: parity_orders)
              for (auto style: parity_styles)
                {
                  auto output = spot::change_parity(aut, order, style);
                  assert(is_right_parity(output, order, style,
      ...
      for (unsigned right_index = 0; right_index < num_right; ++right_index)
        {
          ...
          assert(is_colorized(result_prod, true)
                 && "parity_product: not colorized.");
          assert(is_right_parity(result_prod, spot::parity_order_dontcare,
                                 spot::parity_style_dontcare,
                                 true, true, 2)
          ...
          assert(is_colorized(result_sum, true)
                 && "parity_product_or: not colorized.");
          assert(is_right_parity(result_sum, spot::parity_order_dontcare,
                                 spot::parity_style_dontcare,
                                 true, true, 2)
