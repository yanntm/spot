...
namespace spot
{
  ...
  struct SPOT_API rsscc_nosharing_stats
  {
  ...
  /// rsscc_nosharing inspired from STTT'16 and ATVA'16. It uses a shared
  /// union-find to tag states as dead.
  template<typename State, typename SuccIterator,
           typename StateHash, typename StateEqual>
  class swarmed_rsscc_nosharing
  {
  public:

    swarmed_rsscc_nosharing(kripkecube<State, SuccIterator>& sys,
    ...
    void run()
    {
      ...
      // Insert the initial state
      {
        ...
        todo_.back().isterm = !todo_.back().it->naturally_expanded();
          ...
          if (todo_.back().it->done())
            {
              ...
              // of an SCC.
              if (todo_.back().isterm  && todo_.size()-1 == Rp_.back().second)
                {
