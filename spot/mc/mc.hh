...
namespace spot
{
  ...
  /// \brief Perform the SCC computation algorithm of
  /// bloemen_none.16.ppopp
  template<typename kripke_ptr, typename State,
           typename Iterator, typename Hash, typename Equal>
  static std::pair<std::vector<bloemen_none_stats>, spot::timer_map>
  bloemen_none(kripke_ptr sys)
  {
    spot::timer_map tm;
    using algo_name =
      spot::swarmed_bloemen_none<State, Iterator, Hash, Equal>;
    using uf_name = spot::iterable_uf<State, Hash, Equal>;
    ...
    std::vector<bloemen_none_stats> stats;
    for (unsigned i = 0; i < sys->get_threads(); ++i)
      stats.push_back(swarmed[i]->stats());
  ...
  /// \brief Perform a laarman_source POR exploration (duret.16.atva)
  /// with swarming
  template<typename kripke_ptr, typename State,
           typename Iterator, typename Hash, typename Equal>
  static std::pair<std::vector<laarman_source_stats>, spot::timer_map>
  laarman_source(kripke_ptr sys)
  {
    spot::timer_map tm;
    using algo_name =
      spot::swarmed_laarman_source<State, Iterator, Hash, Equal>;
    using store_name = spot::store<State, Hash, Equal>;
    ...
    std::vector<laarman_source_stats> stats;
    for (unsigned i = 0; i < sys->get_threads(); ++i)
      stats.push_back(swarmed[i]->stats());
  ...
  /// \brief Perform a laarman_safety POR exploration (duret.16.atva)
  /// with swarming
  template<typename kripke_ptr, typename State,
           typename Iterator, typename Hash, typename Equal>
  static std::pair<std::vector<laarman_safety_stats>, spot::timer_map>
  laarman_safety(kripke_ptr sys)
  {
    spot::timer_map tm;
    using algo_name =
      spot::swarmed_laarman_safety<State, Iterator, Hash, Equal>;
    using store_name = spot::store<State, Hash, Equal>;
    ...
    std::vector<laarman_safety_stats> stats;
    for (unsigned i = 0; i < sys->get_threads(); ++i)
      stats.push_back(swarmed[i]->stats());
  ...
  renault_cond_source(kripke_ptr sys)
  {
    ...
    /// \brief Perform a renault_safety POR exploration
  /// (duret.16.atva and renault.16.sttt) with swarming
  template<typename kripke_ptr, typename State,
           typename Iterator, typename Hash, typename Equal>
  static std::pair<std::vector<renault_safety_stats>, spot::timer_map>
  renault_safety(kripke_ptr sys)
  {
    spot::timer_map tm;
    using algo_name =
      spot::swarmed_renault_safety<State, Iterator, Hash, Equal>;
    using uf_name = spot::uf<State, Hash, Equal>;
    ...
    std::vector<renault_safety_stats> stats;
    for (unsigned i = 0; i < sys->get_threads(); ++i)
      stats.push_back(swarmed[i]->stats());
    ...
    /// \brief Perform the SCC computation algorithm of bloemen_safety.16.ppopp
  template<typename kripke_ptr, typename State,
           typename Iterator, typename Hash, typename Equal>
  static std::pair<std::vector<bloemen_safety_stats>, spot::timer_map>
  bloemen_safety(kripke_ptr sys)
  {
    spot::timer_map tm;
    using algo_name
      = spot::swarmed_bloemen_safety<State, Iterator, Hash, Equal>;
    using uf_name = spot::iterable_uf_safety<State, Hash, Equal>;
    ...
    std::vector<bloemen_safety_stats> stats;
    for (unsigned i = 0; i < sys->get_threads(); ++i)
      stats.push_back(swarmed[i]->stats());
  ...
  /// \brief Perform a cond_dest_liveness POR exploration
  /// (duret.16.atva) with swarming
  template<typename kripke_ptr, typename State,
           typename Iterator, typename Hash, typename Equal>
  static std::pair<std::vector<cond_dest_liveness_stats>, spot::timer_map>
  cond_dest_liveness(kripke_ptr sys)
  {
    spot::timer_map tm;
    using algo_name =
      spot::swarmed_cond_dest_liveness<State, Iterator, Hash, Equal>;
    using store_name = spot::store<State, Hash, Equal>;
    ...
    std::vector<cond_dest_liveness_stats> stats;
    for (unsigned i = 0; i < sys->get_threads(); ++i)
      stats.push_back(swarmed[i]->stats());
  ...
  /// \brief Perform a cond_dest_safety POR exploration
  /// with swarming
  template<typename kripke_ptr, typename State,
           typename Iterator, typename Hash, typename Equal>
  static std::pair<std::vector<cond_dest_safety_stats>, spot::timer_map>
  cond_dest_safety(kripke_ptr sys)
  {
    spot::timer_map tm;
    using algo_name =
      spot::swarmed_cond_dest_safety<State, Iterator, Hash, Equal>;
    using store_name = spot::store<State, Hash, Equal>;
    ...
    std::vector<cond_dest_safety_stats> stats;
    for (unsigned i = 0; i < sys->get_threads(); ++i)
      stats.push_back(swarmed[i]->stats());
  ...
  /// \brief Perform a rsscc_nosharing POR exploration
  /// with swarming
  template<typename kripke_ptr, typename State,
           typename Iterator, typename Hash, typename Equal>
  static std::pair<std::vector<rsscc_nosharing_stats>, spot::timer_map>
  rsscc_nosharing(kripke_ptr sys)
  {
    spot::timer_map tm;
    using algo_name =
      spot::swarmed_rsscc_nosharing<State, Iterator, Hash, Equal>;
    using uf_name = spot::uf<State, Hash, Equal>;
    ...
    std::vector<rsscc_nosharing_stats> stats;
    for (unsigned i = 0; i < sys->get_threads(); ++i)
      stats.push_back(swarmed[i]->stats());
  ...
  /// \brief Perform a rsscc_none POR exploration
  /// with swarming
  template<typename kripke_ptr, typename State,
           typename Iterator, typename Hash, typename Equal>
  static std::pair<std::vector<rsscc_none_stats>, spot::timer_map>
  rsscc_none(kripke_ptr sys)
  {
    spot::timer_map tm;
    using algo_name =
      spot::swarmed_rsscc_none<State, Iterator, Hash, Equal>;
    using uf_name = spot::uf<State, Hash, Equal>;
    ...
    std::vector<rsscc_none_stats> stats;
    for (unsigned i = 0; i < sys->get_threads(); ++i)
      stats.push_back(swarmed[i]->stats());
  ...
  /// \brief Perform a rsscc POR exploration
  /// with swarming
  template<typename kripke_ptr, typename State,
           typename Iterator, typename Hash, typename Equal>
  static std::pair<std::vector<rsscc_stats>, spot::timer_map>
  rsscc(kripke_ptr sys)
  {
    spot::timer_map tm;
    using algo_name =
      spot::swarmed_rsscc<State, Iterator, Hash, Equal>;
    using uf_name = spot::uf<State, Hash, Equal>;
