...
/// \bief It is the same algorithm as before but in a distributed context.
/// Each process performs the algorithm sequentially
/// and notifies the others when it detects a deadlock.
/// The processes are then stopped.
template <typename kripke_ptr, typename State, typename Iterator, typename Hash,
          typename Equal>
static std::tuple<bool, std::vector<deadlock_stats>, spot::timer_map>
distributed_has_deadlock(kripke_ptr sys, spot::process* proc)
{
  spot::timer_map tm;
  using algo_name = spot::swarmed_deadlock<State, Iterator, Hash, Equal>;
      ...
      for (unsigned i = 0; i < nbth; ++i)
        {
          threads[i] = std::thread([&swarmed, &iomutex, i, &barrier, &proc] {
            ...
            {
              std::lock_guard<std::mutex> iolock(iomutex);
              proc->out << "Thread #" << i << ": on CPU " << sched_getcpu()
                        << '\n';
            }
#endif
          ...
          if (rc != 0)
            {
              std::lock_guard<std::mutex> iolock(iomutex);
              proc->err << "Error calling pthread_setaffinity_np: " << rc
                        << '\n';
            }
#endif
        }
  ...
  /* As we are in a distributed context
     launching a single thread per process would not make sense,
     so it is the process that directly performs the search for deadlocks. */
  if (nbth == 1)
    {
      tm.start("Initialisation");
