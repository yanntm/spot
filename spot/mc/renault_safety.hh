...
namespace spot
{
  ...
  struct SPOT_API renault_safety_stats
  {
  ...
  /// renault_safety inspired from STTT'16 and ATVA'16. It uses a shared
  /// union-find to tag states as dead.
  template<typename State, typename SuccIterator,
           typename StateHash, typename StateEqual>
  class swarmed_renault_safety
  {
  public:

    swarmed_renault_safety(kripkecube<State, SuccIterator>& sys,
    ...
    void run()
    {
      ...
      while (!todo_.empty())
        {
          // Do we want to share expanded states?
          ...
          if (todo_.back().it->naturally_expanded())
            {
              ...
              atomic_fetch_or(&(todo_.back().e->expanded_local_), w_id);
            }
          ...
          if (todo_.back().it->done())
            {
              ...
              // Not expanded AND we are about to backtrack a root
              // of an SCC.
              if (!(todo_.back().e->expanded_local_.load() & w_id) &&
                  todo_.size()-1 == Rp_.back().second)
                {
                  atomic_fetch_or(&(todo_.back().e->expanded_local_), w_id);
                  todo_.back().it->fireall();
                  continue;
              ...
              if (todo_.size() == Rp_.back().second)
                {
                  ...
                  if (todo_.size() && (s->expanded_local_.load() & w_id))
                    {
                      atomic_fetch_or(&(s->expanded_local_), w_id);
                    }
                }
            }
          else
            {
              ...
              if (w.first == st_uf::claim_status::CLAIM_NEW)
                ...
                {
                  if ((todo_.back().e->expanded_local_ & w_id) ||
                      (w.second->expanded_local_.load() & w_id))
                    {
                      atomic_fetch_or(&(todo_.back().e->expanded_local_), w_id);
                      atomic_fetch_or(&(w.second->expanded_local_), w_id);
                    }
