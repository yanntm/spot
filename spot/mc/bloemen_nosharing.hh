...
namespace spot
{
  ...
  struct SPOT_API bloemen_nosharing_stats
  {
  ...
  /// bloemen_nosharing as described in PPOPP'16. It uses a shared
  /// union-find augmented to manage work stealing between threads.
  template<typename State, typename SuccIterator,
           typename StateHash, typename StateEqual>
  class SPOT_API swarmed_bloemen_nosharing
  {
  public:

    swarmed_bloemen_nosharing(kripkecube<State, SuccIterator>& sys,
    ...
    void run()
    {
      ...
      while (!todo_.empty())
        {
        bloemen_nosharing_recursive_start:
          while (true)
            {
              bool sccfound = false;
              ...
              bool is_expanded = it->naturally_expanded();
                  ...
                  if (w.first == uf::claim_status::CLAIM_NEW)
                    {
                      ...
                      goto bloemen_nosharing_recursive_start;
                    }
