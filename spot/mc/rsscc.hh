...
namespace spot
{
  ...
  public:
    ...
    swarmed_rsscc(kripkecube<State, SuccIterator>& sys,
      ...
      while (!todo_.empty())
        {
          if (todo_.back().it->done())
            {
              // Another thread required an expansion
              if (todo_.back().isterm && todo_.back().e->expanded_)
                {
                  todo_.back().isterm = false;
