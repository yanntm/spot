...
namespace spot
{
  ...
  struct SPOT_API cond_dest_safety_stats
  {
  ...
  /// \brief This class implements a swarmed cond_dest_safety as described in
  // ATVA'16. It uses a shared store to share information between threads.
  template<typename State, typename SuccIterator,
           typename StateHash, typename StateEqual>
  class SPOT_API swarmed_cond_dest_safety
  {
  public:

    swarmed_cond_dest_safety(kripkecube<State, SuccIterator>& sys,
    ...
    using por_status = typename st_store::por_status;
    ...
    void run()
    {
      ...
      // Insert the initial state
      {
        ...
        if (todo_.back().it->naturally_expanded())
          todo_.back().e->por_status_ = por_status::EXPANDED;
      ...
      while (!todo_.empty())
        {
          if (todo_.back().it->done())
            {
              por_status expected = por_status::UNKNOWN;
              if (todo_.back().allin && todo_.back().selection != nullptr)
                {
                  std::atomic_compare_exchange_strong
                    (&(todo_.back().selection->por_status_),
                     &expected, por_status::EXPANDED);
                }

              std::atomic_compare_exchange_strong
                (&(todo_.back().e->por_status_),
                 &expected, por_status::NOT_EXPANDED);

              if (todo_.back().e->por_status_.load() == por_status::EXPANDED &&
                  !todo_.back().it->already_fired()) // only one  expansion!
                {
              ...
              if (w.first == st_store::claim_status::CLAIM_NEW)
              ...
              else if (w.first == st_store::claim_status::CLAIM_FOUND)
                {
                  ...
                  if ((!(w.second->onstack_.load() & w_id)) ||
                      w.second->por_status_.load() != por_status::UNKNOWN ||
                      todo_.back().e->por_status_.load() != por_status::UNKNOWN)
                    {
                      todo_.back().allin = false;
                    }
                  else if (todo_.back().selection == nullptr)
                    {
                      todo_.back().selection = w.second;
                    }
                }
            }
        }
