...
namespace spot
{
  ...
  public:
    ...
    {
      SPOT_ASSERT(e1 < (int)id.size() && e2 < (int)id.size());
  ...
  public:
    ...
    void push_state(const state* st)
    {
      ...
      if (ComputeSCC)
        {
          ...
          SPOT_ASSERT(seen[st].live_number == dfs_number);
      ...
      if (Anticipated || FullyAnticipated)
        {
          SPOT_ASSERT(!ReverseAnticipated);
      ...
      if (ReverseAnticipated)
        {
          SPOT_ASSERT(!Anticipated && !FullyAnticipated);
          static seen_map* seen_ptr;
          ...
          else
            {
              ...
              if (res.second)
              ...
              else
                {
                  ...
                  if (toexpand != -1)
                    {
                      SPOT_ASSERT(toexpand < (int) todo.size());
