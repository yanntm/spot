...
namespace spot
{
  namespace
  {
    ...
    static
    std::ostream&
    operator<<(std::ostream& os, const iar_state& s)
    {
      os << '(' << s.state << ",[";
      for (unsigned i : s.perm)
        os << i << ',';
      os << "])";
      return os;
    }
    */
    ...
    public:
      ...
      {
        // TODO determine the relevant Rabin pairs
            ...
            for (const auto& e : aut_->out(current.state))
              {
                ...
                if (scc_.scc_of(e.dst) != scc_num)
                ...
                else
                  {
                    ...
                    // TODO consider the pairs for this SCC only
                    for (unsigned k = 0; k != pairs_.size(); ++k)
        ...
        unsigned bscc = -1U;
        for (unsigned s = 0; s != sub_scc.scc_count(); ++s)
          {
  ...
  twa_graph_ptr
  rabin_to_parity(const const_twa_graph_ptr& aut)
  {
    if (!is_deterministic(aut))
      throw std::runtime_error("rabin2parity works only for deterministic "
          "automata");

    std::vector<acc_cond::rs_pair> rabin_pairs;
    if (!aut->acc().is_rabin_like(rabin_pairs))
      throw std::runtime_error("rabin2parity works only for Rabin automata");
