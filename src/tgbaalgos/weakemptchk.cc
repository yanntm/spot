...
namespace spot
{
  namespace
  {
    ...
    class weakemptchk_search : public emptiness_check, public ec_statistics
    {
    public:
      ...
      weakemptchk_search(const tgba *a, size_t size, option_map o = option_map())
	: emptiness_check(a, o),
	  h(size),
	...
	if (st_red.empty())
	  {
	    ...
	    if (static_persistence ())
	      return new weakemptchk_result(*this,
					    options());
	  }
	else
	  {
	    ...
	    if (!st_red.empty() && dfs_red())
	      return new weakemptchk_result(*this, options());
	    else
	      if (dfs_blue())
		return new weakemptchk_result(*this, options());
	  }
	...
	while (!st_blue.empty())
	  {
		...
		//
		// Warning : this condition work on weak scc but
	...
	public acss_statistics
      {
      public:
	result_from_stack(weakemptchk_search& ms)
	...
	unsigned acss_states() const
	{
      ...
      private:
	weakemptchk_search& ms_;
      };

#     define FROM_STACK "ar:from_stack"

      class weakemptchk_result: public emptiness_check_result
      {
      public:
	weakemptchk_result(weakemptchk_search& m, option_map o = option_map())
	  : emptiness_check_result(m.automaton(), o), ms(m)
	{
	  if (options()[FROM_STACK])
	    ...
	    computer =
	      new ndfs_result<weakemptchk_search<heap>, heap>(ms);
	}
	...
	weakemptchk_search& ms;
      };
    };

    class explicit_weakemptchk_search_heap
    {
	...
	color *pc; // point to the color of a state stored in main hash table
      ...
      explicit_weakemptchk_search_heap(size_t)
	{
	}

      ~explicit_weakemptchk_search_heap()
	{
    ...
    class bsh_weakemptchk_search_heap
    {
    private:
      typedef Sgi::hash_set<const state*,
		state_ptr_hash, state_ptr_equal> hcyan_type;
	...
	unsigned char o;
      ...
      bsh_weakemptchk_search_heap(size_t s) : size_(s)
	{
	  h = new unsigned char[size_];
	  memset(h, WHITE, size_);
	}

      ~bsh_weakemptchk_search_heap()
	{
	  delete[] h;
	}
  ...
  // 					bool stat)
  // {
  //   return new se05_search<explicit_weak_search_heap>(a, 0, o);
  // }
  ...
  emptiness_check*
  weak_dfs_search(const tgba *a, option_map o)
  {
    //    size_t size = o.get("bsh");
    //return weakemptchk_search_heap(a, o);
    return new weakemptchk_search<explicit_weakemptchk_search_heap>(a, 0, o);
  }
}
