...
namespace spot
{
  namespace
  {
    ...
    class ell_search : public emptiness_check, public ec_statistics
    {
    public:
      ...
      ell_search(const tgba *a, size_t size, option_map o = option_map())
	: emptiness_check(a, o),
	  h(size)
      {
      }
	...
	if (st_red.empty())
	  {
	    ...
	    if (dfs_blue())
	      return new ell_result(*this, options());
	  }
	else
	  {
	    h.pop_notify(st_red.front().s);
	...
	public acss_statistics
      {
      public:
	result_from_stack(ell_search& ms)
...
#     define FROM_STACK "ar:from_stack"
      ...
      class ell_result: public emptiness_check_result
      {
      public:
	ell_result(ell_search& m, option_map o = option_map())
	  : emptiness_check_result(m.automaton(), o), ms(m)
	{
	  if (options()[FROM_STACK])
	    ...
	    computer =
	      new ndfs_result<ell_search<heap>, heap>(ms);
	}
    ...
    class explicit_ell_search_heap
    {
	...
	color *pc; // point to the color of a state stored in main hash table
      ...
      explicit_ell_search_heap(size_t)
	{
	}

      ~explicit_ell_search_heap()
	{
	...
	unsigned char o;
      ...
      bsh_ell_search_heap(size_t s) : size_(s)
	{
  ...
  emptiness_check* explicit_ell_search(const tgba *a,
					    option_map o)
  {
    return new ell_search<explicit_ell_search_heap>(a, 0, o);
  }

  emptiness_check* bit_state_hashing_ell_search(const tgba *a, size_t size,
						     option_map o)
  {
    return new ell_search<bsh_ell_search_heap>(a, size, o);
  }

  emptiness_check*
  ell(const tgba *a, option_map o)
  {
    size_t size = o.get("bsh");
    if (size)
      return bit_state_hashing_ell_search(a, size, o);
    return explicit_ell_search(a, o);
  }
}
