...
{
  bool
  fas::operator()(unsigned int src, unsigned int dst)
  {
    // States are ordered from 0, ..., n. The transitions form j -> i with
    // j < i belong to the fas.  States that were not encountered are not
  ...
  void
  fas::print(std::ostream& os)
  {
    os << ordered_states.size() << '\n';
    for (auto pair : ordered_states)
  ...
  fas::build()
  {
    // Counter associated with number of states
    unsigned order = 0;
    // Number of SCCs in aut_
    unsigned scc_count = scc_.scc_count();
    for (unsigned current_scc = 0; current_scc < scc_count; ++current_scc)
      {
        std::list<unsigned int> state_list = scc_.states_of(current_scc);
        std::vector<bool> to_keep(aut_->num_states(), false);
        for (auto s : state_list)
          to_keep[s] = true;

        // Create SCC component, initial state is any of the SCC
        const spot::const_tgba_digraph_ptr& scc_aut =
          mask_keep_states(aut_, to_keep, state_list.front());
    ...
    while (counter)
      {
        ...
        while ((sink = bdg.pop_sink()))
          {
            unsigned int tgba_sink = bdg.get_tgba_state(sink);
        ...
        while ((source = bdg.pop_source()))
          {
            unsigned int tgba_source = bdg.get_tgba_state(source);
        ...
        if (delta)
          {
            unsigned int tgba_delta = bdg.get_tgba_state(delta);
  ...
  fas::fas(const spot::const_tgba_digraph_ptr& g)
    : aut_(g), scc_(g)
  {
  }
}
