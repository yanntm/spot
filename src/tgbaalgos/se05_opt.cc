...
namespace spot
{
  namespace
  {
    ...
    class se05_opt_search : public emptiness_check, public ec_statistics
    {
    public:
	...
	if (st_red.empty())
	  {
	    ...
	    if (dfs_blue())
	      return new se05_opt_result(*this, options());
	  }
	else
	  {
	    h.pop_notify(st_red.front().s);
	...
	while (!st_blue.empty())
	  {
		...
		// Track terminal
		if ((acc & terminal_acc) == terminal_acc)
		  {
		    if (c.is_white ())
		      {
			inc_states();
		...
		else if ((acc & all_cond) != bddfalse &&
			 (acc & strong_acc) != bddfalse &&
			 c.get_color() != RED)
		...
		assert(!c.is_white());
		if (!st_blue.empty() &&
		    (f_dest.acc & all_cond) != bddfalse &&
		    (f_dest.acc & strong_acc) != bddfalse &&
		    c.get_color() != RED)
		// if (!st_blue.empty() &&
	...
	public acss_statistics
      {
      public:
	result_from_stack(se05_opt_search& ms)
...
#     define FROM_STACK "ar:from_stack"
      ...
      class se05_opt_result: public emptiness_check_result
      {
      public:
	se05_opt_result(se05_opt_search& m, option_map o = option_map())
	  : emptiness_check_result(m.automaton(), o), ms(m)
	{
	  if (options()[FROM_STACK])
	    ...
	    computer =
	      new ndfs_result<se05_opt_search<heap>, heap>(ms);
	}
    ...
    class explicit_se05_opt_search_heap
    {
	...
	color *pc; // point to the color of a state stored in main hash table
      ...
      explicit_se05_opt_search_heap(size_t)
	{
	}

      ~explicit_se05_opt_search_heap()
	{
    ...
    class bsh_se05_opt_search_heap
    {
    private:
      typedef Sgi::hash_set<const state*,
		state_ptr_hash, state_ptr_equal> hcyan_type;
	...
	unsigned char o;
      ...
      bsh_se05_opt_search_heap(size_t s) : size_(s)
	{
	  h = new unsigned char[size_];
	  memset(h, WHITE, size_);
	}

      ~bsh_se05_opt_search_heap()
	{
	  delete[] h;
	}
  ...
  emptiness_check* explicit_se05_opt_search(const tgba *a,
					    option_map o)
  {
    return new se05_opt_search<explicit_se05_opt_search_heap>(a, 0, o);
  }

  emptiness_check* bit_state_hashing_se05_opt_search(const tgba *a, size_t size,
						     option_map o)
  {
    return new se05_opt_search<bsh_se05_opt_search_heap>(a, size, o);
  }

  emptiness_check*
  se05_opt(const tgba *a, option_map o)
  {
    size_t size = o.get("bsh");
    if (size)
      return bit_state_hashing_se05_opt_search(a, size, o);
    return explicit_se05_opt_search(a, o);
  }
}
