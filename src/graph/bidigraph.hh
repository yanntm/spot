...
namespace spot
{
  ...
  namespace graph
  {
    ...
    public:
      ...
      /// dictionary.
      bidigraph(const const_tgba_digraph_ptr& g, bool do_loops);
      bidigraph(const const_tgba_digraph_ptr& g, bool do_loops,
                spot::scc_info* scc, int scc_index);
      ~bidigraph();
      void remove_state(bidistate* s);
      // Initialise graph, and build it from const_tgba_digraph_ptr
      void build();

      // TODO use vector instead of map to get bidistate from tgba_state
      bidistate* get_bidistate(unsigned int tgba_state) const;

      unsigned int get_tgba_state(bidistate* bidistate) const;
    ...
    private:
      const const_tgba_digraph_ptr aut_;
      ...
      std::unordered_map<bidistate*, unsigned int> bidistate_to_tgba_;
      // Each index in vector represents a state from the tgba.
      std::vector<bidistate*> tgba_to_bidistate_;

      // Decrement the in degree of all the states in succ and update bins.
      void update_succs(bidigraph_states* succ);
      // Decrement the out degree of all states in pred and update bins.
      void update_preds(bidigraph_states* pred);
      void create_bidistate(unsigned int tgba_state);
      //  Breadth first search to compute the states of the bidigraph.
      void bfs();
      void remove_delta(bidistate* s, int step);

      // Compute the set of all the bdd (condition set) of a specific scc
      // component
      std::unordered_set<bdd, bdd_hash>
      scc_cond_set_of(unsigned scc) const;
