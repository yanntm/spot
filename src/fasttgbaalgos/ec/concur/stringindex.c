...
static void next_resize(void*arg,void*old_array,int old_size,void*new_array,int new_size){
    SDEBUG("skip next resize from %d to %d",old_size,new_size);
    (void)arg;
    (void)old_array;
    (void)old_size;
    (void)new_array;
...
static void len_resize(void*arg,void*old_array,int old_size,void*new_array,int new_size){
    SDEBUG("extend during len resize from %d to %d",old_size,new_size);
    (void)old_array;
    (void)new_array;
	...
	if ((si->mask*FILL_OUTOF)<(si->count*FILL_MAX)){
		...
		N=si->mask+1;
		SDEBUG("resizing table from %d to %d",N,N+N);
		si->mask=(si->mask<<1)+1;
		...
		for(i=0;i<N;i++){
			...
			while(current!=END_OF_LIST){
				...
				SDEBUG("moving %s from %d to %d",si->data[current],i,bucket);
				current=next;
			}
		}
	}
