...
static int hti_copy_entry (hti_t *ht1, volatile entry_t *ht1_ent,
    ...
    if (HRE_EXPECT_FALSE(ht1_ent_val == COPIED_VALUE || ht1_ent_val == TAG_VALUE(TOMBSTONE, TAG1))) {
...
static map_val_t hti_cas (hti_t *hti, map_key_t key, uint32_t key_hash,
    ...
    if (HRE_EXPECT_FALSE(expected != CAS_EXPECT_WHATEVER && expected != ent_val)) {
        if (HRE_EXPECT_FALSE(expected != (old_existed ? CAS_EXPECT_EXISTS : CAS_EXPECT_DOES_NOT_EXIST))) {
...
static map_val_t hti_get (hti_t *hti, map_key_t key, uint32_t key_hash) {
    ...
    map_val_t ent_val = ent->val;
    if (HRE_EXPECT_FALSE(IS_TAGGED(ent_val, TAG1))) {
        if (HRE_EXPECT_FALSE(ent_val != COPIED_VALUE && ent_val != TAG_VALUE(TOMBSTONE, TAG1))) {
            hti_t *n = ((hti_t)atomic_read(hti)).next;
