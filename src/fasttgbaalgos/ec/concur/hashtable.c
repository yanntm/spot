 ...
 *
 * Note: This is code uses synchronous atomic operations because that
 * is all that x86 provides.
 * Every atomic operation is also an implicit full memory barrier.
 * The upshot is that it simplifies
 * the code a bit, but it won't be as fast as it could be on
 * platforms that provide weaker
...
#ifndef NBD32
// low-order 48 bits is a pointer to a nstring_t
#define GET_PTR(x) ((void *)((x) & MASK(48)))
#else
...
// Lookup <key> in <hti>.
//
// Return the entry that <key> is in, or if <key> isn't in <hti> return
// the entry that it would be
// in if it were inserted into <hti>.
// If there is no room for <key> in <hti> then return NULL, to
// indicate that the caller should look in <hti->next>.
//
// Record if the entry being returned is empty.
// Otherwise the caller will have to waste time
// re-comparing the keys to confirm that it did not lose a race
// to fill an empty entry.
static
volatile entry_t *hti_lookup (hti_t *hti, map_key_t key, uint32_t key_hash,
			      int *is_empty)
{
  ...
  *is_empty = 0;
	  ...
	  if (ent_key == DOES_NOT_EXIST)
	    {
	      ...
	      // indicate an empty so the caller avoids an expensive key compare
	      *is_empty = 1;
	      return ent;
	    }

	  // Compare <key> with the key in the entry.
	  if (HRE_EXPECT_TRUE(hti->ht->key_type->cmp == NULL))
	    {
	      // fast path for integer keys
	      if (ent_key == key) {
		TRACE("h1",
		      "hti_lookup: found entry %p with key %p", ent, ent_key);
		return ent;
	      }
	    }
	  else
	    {
#ifndef NBD32
	      // The key in <ent> is made up of two parts.
	      // The 48 low-order bits are a pointer. The
	      // high-order 16 bits are taken from the hash.
	      // The bits from the hash are used as a
	      // quick check to rule out non-equal keys without doing
	      // a complete compare.
	      if ((key_hash >> 16) == (ent_key >> 48)) {
#endif
		if (hti->ht->key_type->cmp(GET_PTR(ent_key),
					   (void *)key) == 0) {
		  TRACE("h1",
			"hti_lookup: found entry %p with key %p",
			ent, GET_PTR(ent_key));
		  return ent;
#ifndef NBD32
		}
  ...
  TRACE("h1",
	"hti_lookup: maximum number of probes exceeded returning 0x0", 0, 0);
  return NULL;
}
...
static void hti_start_copy (hti_t *hti) {
  ...
  new_scale += (count > (1ULL << (hti->scale - 1)))
    || (hti->key_count > (1ULL << (hti->scale - 2)) +
	(1ULL << (hti->scale - 3))); // double size if more than 1/2 full

  Debug ("Resizing table from %llu to %llu",
	 (1ULL << hti->scale), (1ULL << new_scale));
  ...
  if (old_next != NULL) {
    ...
    TRACE("h0", "hti_start_copy: lost race to install new hti; found %p",
	  old_next, 0);
...
// Copy the key and value stored in <ht1_ent>
// (which must be an entry in <ht1>) to <ht2>.
//
// Return 1 unless <ht1_ent> is already copied (then return 0),
// so the caller can account for the total
...
static int hti_copy_entry (hti_t *ht1, volatile entry_t *ht1_ent,
  ...
  HREassert(ht1->next);
  HREassert(ht2);
  HREassert(ht1_ent >= ht1->table &&
	    ht1_ent < ht1->table + (1ULL << ht1->scale));
#ifndef NBD32
  ...
  if (HRE_EXPECT_FALSE(ht1_ent_val == COPIED_VALUE ||
		       ht1_ent_val == TAG_VALUE(TOMBSTONE, TAG1))) {
    TRACE("h1", "hti_copy_entry: entry %p already copied to table %p",
	  ht1_ent, ht2);
    return false; // already copied
  }

  // Kill empty entries.
  if (HRE_EXPECT_FALSE(ht1_ent_val == DOES_NOT_EXIST)) {
    map_val_t ht1_ent_val =
      cas_ret(&ht1_ent->val, DOES_NOT_EXIST, COPIED_VALUE);
    if (ht1_ent_val == DOES_NOT_EXIST) {
      TRACE("h1", "hti_copy_entry: empty entry %p killed", ht1_ent, 0);
      return true;
    }
    TRACE("h0", "hti_copy_entry: lost race to kill empty entry %p;\
 the entry is not empty", ht1_ent, 0);
  }

  // Tag the value in the old entry to indicate a copy is in progress.
  ht1_ent_val = fetch_or(&ht1_ent->val, TAG_VALUE(0, TAG1));
  TRACE("h2", "hti_copy_entry: tagged the value %p in old entry %p",
	ht1_ent_val, ht1_ent);
  ...
  map_key_t key = (ht1->ht->key_type == NULL) ?
    (map_key_t)ht1_ent_key : (map_key_t)GET_PTR(ht1_ent_key);

  // We use 0 to indicate that <key_hash> is uninitialized.
  // Occasionally the key's hash will really be 0 and we
  // waste time recomputing it every time.
  // It is rare enough that it won't hurt performance.
  if (key_hash == 0) {
    key_hash = hash32(ht1->ht, key, ctx);
  }

  int ht2_ent_is_empty;
  volatile entry_t *ht2_ent =
    hti_lookup(ht2, key, key_hash, &ht2_ent_is_empty);
  ...
  if (HRE_EXPECT_FALSE(ht2_ent == NULL)) {
    TRACE("h0",
	  "hti_copy_entry: no room in table %p copy to next table %p",
	  ht2, ht2->next);
    if (ht2->next == NULL) {
      hti_start_copy(ht2); // initiate nested copy, if not already started
    }
    return hti_copy_entry(ht1, ht1_ent,
			  key_hash, ht2->next, ctx); // recursive tail-call
  }

  if (ht2_ent_is_empty) {
    map_key_t old_ht2_ent_key =
      cas_ret(&ht2_ent->key, DOES_NOT_EXIST, ht1_ent_key);
    if (old_ht2_ent_key != DOES_NOT_EXIST) {
      TRACE("h0", "hti_copy_entry: lost race to CAS key %p\
 into new entry; found %p",
	    ht1_ent_key, old_ht2_ent_key);
      return hti_copy_entry(ht1, ht1_ent,
			    key_hash, ht2, ctx); // recursive tail-call
    }
  ...
  map_val_t old_ht2_ent_val =
    cas_ret(&ht2_ent->val, DOES_NOT_EXIST, ht1_ent_val);

  // If there is a nested copy in progress,
  // we might have installed the key into a dead entry.
  if (old_ht2_ent_val == COPIED_VALUE) {
    TRACE("h0",
	  "hti_copy_entry: nested copy in progress; copy %p\
 to next table %p", ht2_ent, ht2->next);
    return hti_copy_entry(ht1, ht1_ent,
			  key_hash, ht2->next, ctx); // recursive tail-call
  }
  ...
  if (old_ht2_ent_val == DOES_NOT_EXIST) {
    TRACE("h0", "hti_copy_entry: key %p value %p copied to new entry",
	  key, ht1_ent_val);
  ...
  TRACE("h0", "hti_copy_entry: lost race to install value %p\
 in new entry; found value %p",
...
// replace the existing value with <new>. If <new> is DOES_NOT_EXIST, delete
// the value associated with
// the key by replacing it with a TOMBSTONE.
//
// Return the previous value associated with <key>, or DOES_NOT_EXIST if <key>
// is not in the table
// or associated with a TOMBSTONE. If a copy is in progress and <key> has been
// copied to the next
// table then return COPIED_VALUE.
//
// NOTE: the returned value matches <expected> iff the set succeeds
//
// Certain values of <expected> have special meaning.
// If <expected> is CAS_EXPECT_EXISTS then any
// real value matches (i.ent. not a TOMBSTONE or DOES_NOT_EXIST) as long
...
static map_val_t hti_cas (hti_t *hti, map_key_t key, uint32_t key_hash,
  ...
  if (is_empty) {
    ...
    if (expected != CAS_EXPECT_WHATEVER &&
	expected != CAS_EXPECT_DOES_NOT_EXIST)
    ...
    if (old_ent_key != DOES_NOT_EXIST) {
      TRACE("h0", "hti_cas: lost race to install key %p in entry %p",
	    new_key, ent);
      TRACE("h0", "hti_cas: found %p instead of NULL",
	    (hti->ht->key_type->clone == NULL) ?
	    (void *)old_ent_key : GET_PTR(old_ent_key), 0);
	...
	nbd_free(GET_PTR(new_key));
      ...
      return hti_cas(hti, key, key_hash,
		     expected, new, clone_key, ctx); // tail-call
    }
    TRACE("h2", "hti_cas: installed key %p in entry %p", new_key, ent);
    add_fetch(&hti->key_count, 1);
  } else {
    *clone_key =
      (map_key_t)GET_PTR(ent->key);
    // we might still win the race to install the
  ...
  TRACE("h0", "hti_cas: entry for key %p is %p",
	(hti->ht->key_type == NULL) ?
	(void *)ent->key : GET_PTR(ent->key), ent);

  // If the entry is in the middle of a copy,
  // the copy must be completed first.
  map_val_t ent_val = ent->val;
  if (HRE_EXPECT_FALSE(IS_TAGGED(ent_val, TAG1))) {
	...
	(void)add_fetch(&hti->num_entries_copied, 1);
      ...
      TRACE("h0", "hti_cas: value in the middle of a copy,\
 copy completed by %s",
	    (did_copy ? "self" : "other"), 0);
    }
    TRACE("h0", "hti_cas: value copied to next table,\
 retry on next table", 0, 0);
    return COPIED_VALUE;
  }
  ...
  if (HRE_EXPECT_FALSE(expected != CAS_EXPECT_WHATEVER &&
		       expected != ent_val)) {
    if (HRE_EXPECT_FALSE(expected != (old_existed ?
				      CAS_EXPECT_EXISTS :
				      CAS_EXPECT_DOES_NOT_EXIST))) {
      TRACE("h1", "hti_cas: value %p expected by\
 caller not found; found value %p",
	    expected, ent_val);
      return ent_val;
    }
  }
  ...
  map_val_t v =
    cas_ret(&ent->val, ent_val, new == DOES_NOT_EXIST ? TOMBSTONE : new);
  if (HRE_EXPECT_FALSE(v != ent_val)) {
    TRACE("h0",
	  "hti_cas: value CAS failed; expected %p found %p", ent_val, v);
    return hti_cas(hti, key, key_hash,
		   expected, new, clone_key, ctx); // recursive tail-call
  }
  ...
  TRACE("h0",
	"hti_cas: CAS succeeded; old value %p new value %p", ent_val, new);
  return ent_val;
}
...
static map_val_t hti_get (hti_t *hti, map_key_t key, uint32_t key_hash) {
  ...
  // If the entry is being copied, finish the copy and retry
  // on the next table.
  map_val_t ent_val = ent->val;
  if (HRE_EXPECT_FALSE(IS_TAGGED(ent_val, TAG1))) {
    if (HRE_EXPECT_FALSE(ent_val != COPIED_VALUE &&
			 ent_val != TAG_VALUE(TOMBSTONE, TAG1))) {
      hti_t *n = ((hti_t)atomic_read(hti)).next;
...
static int hti_help_copy (hti_t *hti, void *ctx) {
  ...
  TRACE("h1", "ht_cas: help copy.\
 scan is %llu, size is %llu", x, 1<<hti->scale);
  if (total_copied != (1ULL << hti->scale)) {
    // Panic if we've been around the array twice and still
    ...
    if (!panic) {
      ...
      // copying. In that case we just wrap around to the begining and
...
static void hti_defer_free (hti_t *hti) {
  ...
  for (uint32_t i = 0; i < (1ULL << hti->scale); ++i) {
    ...
    if (val == COPIED_VALUE)
      continue;
    HREassert(!IS_TAGGED(val, TAG1) ||
	      val == TAG_VALUE(TOMBSTONE, TAG1)); // copy not in progress
...
map_val_t ht_cas (hashtable_t *ht, map_key_t key, map_val_t expected_val,
  ...
  HREassert(!IS_TAGGED(new_val, TAG1) &&
	    new_val != DOES_NOT_EXIST && new_val != TOMBSTONE);

  hti_t *hti = ht->hti;

  // Help with an ongoing copy.
  if (HRE_EXPECT_FALSE(hti->next != NULL)) {
  ...
  while ((old_val = hti_cas(hti, key, key_hash,
			    expected_val, new_val, clone_key, ctx))
	 == COPIED_VALUE) {
    HREassert(hti->next);
    hti = hti->next;
  }
...
// Remove the value in <ht> associated with <key>.
// Returns the value removed, or DOES_NOT_EXIST if there was
...
map_val_t ht_remove (hashtable_t *ht, map_key_t key, map_key_t *clone_key) {
  ...
  uint32_t key_hash = hash32(ht, key, NULL);
  do {
    val = hti_cas(hti, key, key_hash,
		  CAS_EXPECT_WHATEVER, DOES_NOT_EXIST, clone_key, NULL);
    if (val != COPIED_VALUE)
...
void ht_print (hashtable_t *ht, int verbose) {
  printf("probe:%-2d density:%.1f%% count:%-8zu ",
	 ht->probe, ht->density, ht_count(ht));
  hti_t *hti = ht->hti;
  while (hti) {
    if (verbose) {
    ...
    //int scale = hti->scale;
    //printf("hti count:%lld scale:%d key density:%.1f%% value
    // density:%.1f%% probe:%d\n",
    //       (uint64_t)hti->count, scale, (double)hti->key_count /
    // (1ULL << scale) * 100,
...
void ht_iter_value (hashtable_t *ht, process_fun_t process)
{
  ...
  for (size_t i = 1; i < (1ULL << hti->scale); ++i) {
    ...
    if ((size_t)ent->key != DOES_NOT_EXIST )
      {
	//	    printf("[%d] 0x%zu:0x%zu\n",
	//(int)hti->copy_scan, (size_t)ent->key, (size_t)ent->val);
	(*process) ((void*)ent->val);
      }
  }
}
...
map_val_t ht_iter_next (ht_iter_t *iter, map_key_t *key_ptr) {
  ...
  do {
    ...
    key = (iter->hti->ht->key_type->clone == NULL) ?
      (map_key_t)ent->key : (map_key_t)GET_PTR(ent->key);
    val = ent->val;
