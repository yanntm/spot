...
namespace spot
{
  ...
  // RND ONE PROVISO
  // -----------------------------------------------------------------
  bool rnd_one_proviso::expand_new_state(int n,
					 bool expanded,
					 twa_succ_iterator*,
					 seen_map&,
					 const state*,
					 const const_twa_ptr&)
  {
    reach_.insert({n, {false, true, (unsigned) expanded_.size()}});
    expanded_.push_back(expanded);
    to_be_expanded_.push_back(false);
    root_stack_.push_back({n, {n}});
    return false;
  }

  bool rnd_one_proviso::expand_src_closingedge(int, int dst)
  {
    auto idx = reach_.find(dst);
    assert(idx != reach_.end());
    ...
    // The dst is on DFS we must set a state as to be expanded on
    // the path (dst...src)
    unsigned rnd = 0;
    unsigned begin = 0;
    if (idx->second.on_dfs)
      begin = reach_[dst].dfs_id;
    else
      begin = reach_[root_stack_.back().n].dfs_id;

    // Check if there is there is an expanded state on the  path
    for (unsigned i = begin; i < expanded_.size(); ++i)
      if (to_be_expanded_[i] || expanded_[i])
	return false;

    // Otherwise pick one state and mark as to be expanded latter.
    std::mt19937 generator (0);
    rnd = begin +  (generator() % (expanded_.size() - begin));
    to_be_expanded_[rnd] = true;
    return false;
  }

  bool rnd_one_proviso::expand_before_pop(int n, twa_succ_iterator*, seen_map&)
  {
    // This state must be expanded.
    if (to_be_expanded_.back() && !expanded_.back())
      {
	expanded_.back() = true;
	return true;
      }
    ...
    reach_[n].on_dfs = false;
    expanded_.pop_back();
    to_be_expanded_.pop_back();
    return false;
  }

  rnd_one_proviso::~rnd_one_proviso()
  { }

  // -----------------------------------------------------------------
  // MIN SUCC PROVISO
  // -----------------------------------------------------------------
  bool min_succ_proviso::expand_new_state(int n, bool expanded,
					  twa_succ_iterator* it,
					  seen_map&,
					  const state*,
					  const const_twa_ptr&)
  {
    ...
    reach_.insert({n, {false, true, (unsigned) expanded_.size()}});
    expanded_.push_back(expanded);
    succ_.push_back(nb_succ);
    to_be_expanded_.push_back(false);
    root_stack_.push_back({n, {n}});
    return false;
  }

  bool min_succ_proviso::expand_src_closingedge(int, int dst)
  {
    auto idx = reach_.find(dst);
    assert(idx != reach_.end());
	...
	if (to_be_expanded_[i] || expanded_[i])
	  return false;
	if (succ_[i] < succ_[candidate])
	  candidate = i;
      }

    to_be_expanded_[candidate] = true;
    return false;
  }

  bool min_succ_proviso::expand_before_pop(int n,
					   twa_succ_iterator*, seen_map&)
  {
    // This state must be expanded.
    if (to_be_expanded_.back() && !expanded_.back())
  ...
  min_succ_proviso::~min_succ_proviso()
  { }

  // -----------------------------------------------------------------
  // MIN SUCC EN PROVISO
  // -----------------------------------------------------------------
  bool min_succ_en_proviso::expand_new_state(int n, bool expanded,
					     twa_succ_iterator* it,
					     seen_map&,
					     const state* st,
					     const const_twa_ptr& aut)
  {
    ...
    // Now compute the number of enabled successors. To do this
    // we produce a fresh iterator, fire all it's successors and count them
    auto the_begining =  aut->succ_iter(st);
    the_begining->fire_all();
    unsigned nb_succ_en = 0;
    the_begining->first();
    while (!the_begining->done())
      {
	++nb_succ_en;
	the_begining->next();
      }
    assert(nb_succ_en >= nb_succ);
    ...
    // since the cost of firing this state is en - red we store this value.
    succ_.push_back(nb_succ_en - nb_succ);
    to_be_expanded_.push_back(false);
    root_stack_.push_back({n, {n}});
    return false;
  }

  bool min_succ_en_proviso::expand_src_closingedge(int, int dst)
  {
    auto idx = reach_.find(dst);
    assert(idx != reach_.end());
  ...
  bool min_succ_en_proviso::expand_before_pop(int n,
					   twa_succ_iterator*, seen_map&)
  {
    // This state must be expanded.
    if (to_be_expanded_.back() && !expanded_.back())
  ...
  min_succ_en_proviso::~min_succ_en_proviso()
  { }

  // -----------------------------------------------------------------
  // MAX SUCC PROVISO
  // -----------------------------------------------------------------
  bool max_succ_proviso::expand_new_state(int n, bool expanded,
					  twa_succ_iterator* it,
					  seen_map&, const state*,
					  const const_twa_ptr&)
  {
  ...
  bool max_succ_proviso::expand_src_closingedge(int, int dst)
  {
    auto idx = reach_.find(dst);
    assert(idx != reach_.end());
  ...
  bool max_succ_proviso::expand_before_pop(int n,
					   twa_succ_iterator*, seen_map&)
  {
    // This state must be expanded.
    if (to_be_expanded_.back() && !expanded_.back())
  ...
  max_succ_proviso::~max_succ_proviso()
  { }

  // -----------------------------------------------------------------
  // MIN SUCC EN PROVISO
  // -----------------------------------------------------------------
  bool max_succ_en_proviso::expand_new_state(int n, bool expanded,
					     twa_succ_iterator* it,
					     seen_map&,
					     const state* st,
					     const const_twa_ptr& aut)
  ...
  bool max_succ_en_proviso::expand_src_closingedge(int, int dst)
  {
    auto idx = reach_.find(dst);
    assert(idx != reach_.end());
  ...
  bool max_succ_en_proviso::expand_before_pop(int n,
					   twa_succ_iterator*, seen_map&)
  {
    // This state must be expanded.
    if (to_be_expanded_.back() && !expanded_.back())
  ...
  max_succ_en_proviso::~max_succ_en_proviso()
  { }
