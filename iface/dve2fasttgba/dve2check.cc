...
main(int argc, char **argv)
{
  ...
  if (!strncmp("-lc13", argv[3], 5))
  ...
  else if (!strncmp("-tacas13uc13", argv[3], 12))
    {
    ...
    {
      opt_async_dijkstra = true;
      option_concur_ec_dead_dijkstra = "-cs";
      std::string s = std::string(argv[3]+15);
      nb_threads = std::stoi(s);
      assert(nb_threads <= std::thread::hardware_concurrency());
      assert(nb_threads >= 2);
    }
  ...
  if (f1)
	  ...
	  || opt_tacas13_ndfs || opt_tacas13_uc13 || opt_tacas13_tuc13)
	{
	  ...
	  if (opt_tacas13_tarjan || opt_tacas13_dijkstra || opt_tacas13_ndfs
		...
		result << "#tacas13dijkstra"  << option_tacas13 << ",";
	      else if (opt_tacas13_ndfs)
		result << "#tacas13ndfs"  << option_tacas13 << ",";
	      else
		result << "#tacas13uc13"  << option_tacas13 << ",";
	...
	if (opt_async_dijkstra)
	  {
	    result << "#async_dijkstra"
		   << option_concur_ec_dead_mixed
		   << nb_threads << ",";
	    spot::dead_share* d =
	      new spot::dead_share(itor, nb_threads,
	    			   spot::dead_share::ASYNC_DIJKSTRA,
	    			   "");
	    mtimer.start("async_dijkstra");
	    if (d->check())
	      result << "VIOLATED,";
	    else
	      result << "VERIFIED,";
