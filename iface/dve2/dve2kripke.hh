...
namespace spot
{
  ...
  public:
    ...
    // If por attribute is not NONE, the returned iterator is
    // generated by the partial order reduction algorithm.
    //
    // prod_state and prod_tgba arguments are used for the adaptive
    // method to know the current state in the formula automaton
    ...
    // Compute if an operation (start - to) is invisible for the
    // formula.
    //
    // If form_st is not 0, it only looks at the atomic propositions
    // visible from this state of the formula automaton.
  ...
  private:
    ...
    // Partial order reduction method to use.
    por::type por_;

    // Has the scc_map already been computed?
    mutable bool computed_;
    // Used in adaptive partial order to find the visible AP from a
    // given formula state
    mutable scc_map* sccmap_;

    // Keep track of the positions of the process counters and global
    // variables one the variable vector.
