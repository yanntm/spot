...
# aut1, aut2, strength aut1, strength aut2, time prd+ec pec, time prd otf, time ec otf, time prd exp, time ec exp, res pec, res otf, res exp, states prd, trans prd, time ar pec, time ar otf, time ar exp
...
def comp(ids):
    ...
    if res_pec is not None:
        start = time.perf_counter()
        ar_pec = res_pec.left_accepting_run()
        end = time.perf_counter()
        time_ar_pec = end - start
    ...
    return (id1, id2, bool(aut1.prop_weak()), bool(aut2.prop_weak()),
            time_prd_ec_pec, time_prd_otf, time_ec_otf, time_prd_exp, time_ec_exp,
            not bool(res_pec), res_otf, res_exp,
            prd_exp.num_states(), prd_exp.num_edges(),
            time_ar_pec, time_ar_otf, time_ar_exp)
    ...
    with open("bench.log", 'w') as log:
        ...
        with open(name, 'w') as f:
            ...
            writer.writerow(['id1','id2',
                             'strength1','strength2',
                             'time_both_pec','time_prd_otf','time_ec_otf',
                               'time_prd_exp','time_ec_exp',
                             'res_pec','res_otf','res_exp',
